...
class PurchasedVideoPage extends PureComponent<IProps, IStates> {
  ...
  render() {
    ...
    return (
      <>
        <Head>
          <title>{t('My Purchased Videos')}</title>
        </Head>
        <div className="main-profile-background">
          <PageHeader title={t('Purchased Videos')} />
          <div className="purchased-videos-page pad40">
            ...
            {success && transactions.length > 0 ? (
              ...
              <div className="pad20">
                {t('You have not purchased any videos yet')}
              </div>
            )}
          </div>
        </div>
        <div className="popup-video">
          <PopupVideoDetail t={t} ref={(ref) => (this.popupRef = ref)} />
        </div>
      </>
    );
  }
...
export default withTranslation('common')(PurchasedVideoPage);
