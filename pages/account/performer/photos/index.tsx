...
import { IPhoto, ISearch, IResponse, IDataResponse } from 'src/interfaces';
import {
  getMyPhotos,
  removeMyPhoto,
  addMyPhotos
...
class PerformerPhotoPage extends PureComponent<IProps, IStates> {
  ...
  async onRemove(id: string) {
    ...
    try {
      ...
      message.success(this.props.t('Removed!'));
  ...
  async addPhotos() {
    ...
    try {
      ...
      offset = limit + offset;
      const resp: IResponse<IDataResponse<IPhoto>> =
  ...
  render() {
    const { data, searching, total, success, t } = this.props;
            ...
            extra={
              <Space>
                <Button
                  type="primary"
                  onClick={() =>
                    Router.push('/account/performer/galleries/add')
                  }
                >
                  {t('Add a new photo gallery')}
                </Button>
                <Button
                  type="primary"
...
export default connect(
  mapStateToProps,
  mapDispatchs
)(withTranslation('common')(PerformerPhotoPage));
