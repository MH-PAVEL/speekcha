...
class UpdatePerformerGalleryPage extends PureComponent<IProps, IStates> {
  ...
  async onFinish(data) {
    ...
    try {
      ...
      message.success(this.props.t('Update gallery success.'));
    } catch (e) {
      this.showError(e);
    } finally {
  ...
  render() {
    const { gallery, performer, t } = this.props;
    ...
    return (
      <>
        <Head>
          <title>
            {t('Update a Gallery')} {gallery && gallery.name}
          </title>
        </Head>
        <div className="performer-gallries-page">
          <PageHeader title={t('Update a Gallery')} />
          <Form
            loading={onSubmit}
            performer={performer}
...
export default connect(mapStateToProps)(
  withTranslation('common')(UpdatePerformerGalleryPage)
);
