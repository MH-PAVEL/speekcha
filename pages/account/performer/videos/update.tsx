...
class CreatePerformerVideosPage extends PureComponent<IProps, IStates> {
  ...
  async onFinish(data) {
    ...
    try {
      ...
      message.success(this.props.t('Update video success.'));
    } catch (e) {
  ...
  render() {
    ...
    return (
      <>
        <Head>
          <title>
            {t('Update a Video')} -{video && video.title}
          </title>
        </Head>
        <div className="performer-videos-page">
          <PageHeader title={t('Update a Video')} />
          <VideoForm
            loading={onSubmit}
...
export default connect(mapStateToProps)(
  withTranslation('common')(CreatePerformerVideosPage)
);
