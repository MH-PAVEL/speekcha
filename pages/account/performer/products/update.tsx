...
class PerformerProductsPage extends PureComponent<IProps, IStates> {
  ...
  async onFinish(data) {
    ...
    try {
      ...
      message.success(this.props.t('Update product success.'));
    } catch (e) {
  ...
  render() {
    ...
    return (
      <>
        <Head>
          <title>
            {t('Update Product')} - {product && product.name}
          </title>
        </Head>
        <div className="performer-products-page">
          <PageHeader title={t('Update Product')} />
          <ProductForm
            loading={onSubmit}
...
export default connect(mapStateToProps)(
  withTranslation('common')(PerformerProductsPage)
);
