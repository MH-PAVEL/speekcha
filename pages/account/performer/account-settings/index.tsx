...
class UserProfilePage extends PureComponent<IProps, IStates> {
  ...
  componentDidUpdate(prevProps: IProps, prevStates: IStates) {
    ...
    if (prevProps.updateError !== updateError && updateError) {
      message.error(t(getResponseError(updateError)));
    }
    ...
    if (
      ...
      message.error(t(getResponseError(auth.updatePassword.error)));
    }
  ...
  async onUpdateBroadcastSetting(data) {
    ...
    try {
      ...
      message.error(t(getResponseError(err)));
    } finally {
  ...
  async onSuspendAccount(data) {
    ...
    try {
      ...
      message.error(t(getResponseError(error)));
    }
    return undefined;
  }
          ...
          >
            ...
            <Tabs.TabPane tab={t('Payment Information')} key="paymentInfo">
              ...
              >
                ...
                <Panel header={t('Paypal')} key="paypal" forceRender>
                  <PaypalSettingFrom
            ...
            <Tabs.TabPane key="timezone" tab={t('Timezone')}>
              <h3>
                {t(
                  'Sometimes the timezone is very important so make sure you alway set up it correctly'
                )}
                .{' '}
                {t(
                  'We will contact you taking into consideration the time zone and so may the performer do'
                )}
              </h3>
              <Form
                onFinish={this.onFinish.bind(this)}
...
export default connect(
  mapStateToProps,
  mapDispatch
)(withTranslation('common')(UserProfilePage));
