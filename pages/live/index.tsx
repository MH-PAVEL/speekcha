...
class PerformerLivePage extends PureComponent<P, S> {
  ...
  componentDidUpdate(prevProps: P) {
    const { updateSuccess, updateError, t } = this.props;
    if (prevProps.updateSuccess !== updateSuccess && updateSuccess) {
      message.success(t('Update Status Success'));
    }
  ...
  start = async () => {
    try {
      ...
      message.error(this.props.t(getResponseError(error)));
    } finally {
  ...
  async removeAllMessage() {
    ...
    try {
      if (
        !window.confirm(`${t('Are you sure you want to remove chat history')}?`)
      ) {
        return;
      }
