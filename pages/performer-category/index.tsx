...
class PerformerCategoryPage extends PureComponent<IProps, IStates> {
  ...
  render() {
    const { category, pluralTextModel, t } = this.props;
    const { query } = this.state;
        ...
        <Head>
          <title>
            {category
              ...
              : `${t('All')} ${t(pluralTextModel) || t('Performers')}`}
          </title>
        </Head>
        <PageHeader
          title={
            category
              ? t(category.name)
              : `${t('All')} ${pluralTextModel || t('Performers')}`
          }
        />
        <div className="">
          <PerformerGrid
...
export default connect(
  mapStateToProps,
  mapDispatch
)(withTranslation('common')(PerformerCategoryPage));
