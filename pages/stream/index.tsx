...
class LivePage extends PureComponent<IProps, IStates> {
  ...
  modelLeftHandler({ performerId }) {
    ...
    this.subscrbierRef.current?.stop();
    message.info(`${this.props.t('Model has left the room')}!`);
  }
    ...
    switch (type) {
      ...
      case 'product':
        if (isBought && itemType === 'digital') {
          message.info(
            `${this.props.t('You have purchased ')} ${name} ${this.props.t(
              'already. Please check your email'
            )}!`
          );
          return;
        }
...
export default connect(
  mapStateToProps,
  mapDispatch
)(withTranslation('common')(LivePage));
