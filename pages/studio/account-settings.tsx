...
class StudioProfilePage extends PureComponent<P, S> {
  ...
  componentDidUpdate(prevProps: P, prevStates: S) {
    const { success, errorInfo, auth, t } = this.props;
  ...
  async submitPaymentInfoForm(data) {
    try {
      ...
      message.success(this.props.t('Update Payment Information Success'));
    } catch (e) {
  ...
  render() {
    const { action, updating, studio, countries, t } = this.props;
...
export default connect(
  mapStates,
  mapDispatch
)(withTranslation('common')(StudioProfilePage));
