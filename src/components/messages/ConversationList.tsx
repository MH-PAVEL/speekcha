...
class ConversationList extends PureComponent<IProps> {
  ...
  onMessage = async (message: {
    ...
    await messageService.readAllInConversation(
      conversationId,
      currentUser._id ? currentUser._id : currentPerformer._id
    );
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <h4 className="text-center" style={{ fontSize: '22px' }}>
          {t('Messenger')}
        </h4>
        <ConversationSearch
          onSearch={(e) => {
        ...
        {!requesting && !conversations.length && (
          <p className="text-center">{t('No conversation found')}.</p>
        )}
      </div>
    );
  }
...
export default connect(
  mapStates,
  mapDispatch
)(withTranslation('components')(ConversationList));
