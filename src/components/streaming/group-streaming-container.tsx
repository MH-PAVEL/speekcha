...
class GroupStreamingContainer extends PureComponent<IProps, IState> {
  ...
  async onReadyCallback(streamId: string, isPublisher = false) {
    ...
    try {
      ...
      this.players[`player_${streamId}`].on('ended', () =>
        this.ended(streamId)
      );
      this.players[`player_${streamId}`].on('error', () =>
        this.ended(streamId)
      );
  ...
  render() {
    const { onClick, btnText, initialized, publish_started, t } = this.props;
