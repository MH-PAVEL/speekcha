...
export default ({
  ...
  const onLike = async () => {
    if (!checkUserLogin(loggedIn, user)) {
      message.error(t('Please login to add favorite'));
      return;
    }
  ...
  const sendGift = () => {
    if (!checkUserLogin(loggedIn, user)) {
      message.error(`${t('Please login to send gift to')} ${username}!`);
    }
  };

  const notify = () => {
    if (!checkUserLogin(loggedIn, user)) {
      message.error(`${t('Please login to notify')} ${username}!`);
    }
  };
