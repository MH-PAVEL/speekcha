...
class Publisher extends PureComponent<IProps, States> {
  ...
  render() {
    const { initialized, publish_started, t } = this.props;
    const videoProps = {
      id: 'publisher',
      className: 'video-js',
      autoPlay: true,
      ...
      <div style={{ minHeight: 300 }} className="text-center">
        ...
        {publish_started && (
          <Badge
            status="success"
            text={
              <span className="badge-success publishing">
                {t('Publishing')}
              </span>
            }
          />
...
// export default withAntmedia(withTranslation('components')(Publisher));
