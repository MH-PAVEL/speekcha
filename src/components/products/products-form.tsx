...
const ProductForm = ({ onFinish, loading, product }: IProps) => {
  ...
  if (product?.token && !unitPrices.find((p) => p.value === product.token)) {
    unitPrices.push({
      value: product.token,
      text: <NumberFormat value={product.token} suffix={t('Tokens')} />
    });
  ...
  const rightInputFrom: FormItemProps[] = [
    ...
    },
    {
      ...
      rules: [
        {
          required: true,
          message: t('Please input product instock')
        },
        {
          pattern: new RegExp(/^[1-9]\d*$/gm),
          message: t('Stock must be positve integer number')
        }
      ],
      children: <Input type="number" placeholder={t('Enter Product Stock')} />
    }
  ];
    ...
    >
      <Row gutter={25}>
        ...
        <Col sm={12} xs={24}>
          ...
          <Form.Item name="image" label={t('Image File')}>
            <Upload
          ...
          {productType === 'digital' && (
            <>
              <Form.Item name="digitalFile" label={t('Digital File')}>
                <Upload
                  showUploadList={{
