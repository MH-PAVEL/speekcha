import * as React from 'react';
import { Table, Button, Space, Menu, Input } from 'antd';
import {
  EditOutlined,
  DeleteOutlined,
  SearchOutlined
} from '@ant-design/icons';
import { IProduct } from 'src/interfaces';

// import Link from 'next/link';
import { useTranslation, Link } from '../../../i18n';

interface IProps {
  products: IProduct[];
  searching: boolean;
  total: number;
  pageSize: number;
  remove?: Function;
  onChange(pagination, filters, sorter, extra): Function;
}

const ProductsTable = ({
  products,
  searching,
  remove,
  total,
  pageSize,
  onChange
}: IProps) => {
  const { t } = useTranslation('components');
  const columns = [
    {
      title: t('Product Image'),
      dataIndex: 'image',
      key: 'image',
      responsive: ['md'] as any,
      render: (text, product: IProduct) => (
        <Link
          href={{
            pathname: '/account/performer/products/update',
            query: { id: product._id, product: JSON.stringify(product) }
          }}
          as={`/account/performer/products/${product._id}/update`}
        >
          <a>
            <img src={text} alt="" />
          </a>
        </Link>
      )
    },
    {
      title: t('Name'),
      dataIndex: 'name',
      key: 'q',
      render: (text) => (
        <Link href="/">
          <a>{text}</a>
        </Link>
      ),
      filterIcon: (filtered) => (
        <SearchOutlined style={{ color: filtered ? '#ff0066' : undefined }} />
      ),
      filterDropdown: ({ setSelectedKeys, confirm, clearFilters }: any) => (
        <div style={{ padding: 5 }}>
          <Input
            placeholder={t('Product Name')}
            size="large"
            onChange={(event) => {
              if (!event.target.value) {
                setSelectedKeys([]);
                clearFilters();
              }
            }}
            onPressEnter={(event) => {
              setSelectedKeys([event.currentTarget.value]);
              confirm();
            }}
            allowClear
          />
        </div>
      )
    },
    {
      title: t('Type'),
      dataIndex: 'type',
      key: 'type',
      filterIcon: (filtered) => (
        <SearchOutlined style={{ color: filtered ? '#ff0066' : undefined }} />
      ),
      filterDropdown: ({
        setSelectedKeys,
        selectedKeys,
        confirm,
        clearFilters
      }: any) => (
        <div style={{ padding: 5 }}>
          <Menu
            style={{ borderRight: 'none' }}
            onSelect={(param) => {
              setSelectedKeys([param.key]);
              confirm();
            }}
            selectedKeys={selectedKeys}
          >
            <Menu.Item key="physical">{t('Physical')}</Menu.Item>
            <Menu.Item key="digital">{t('Digital')}</Menu.Item>
          </Menu>
          <Button
            onClick={() => {
              setSelectedKeys([]);
              clearFilters();
            }}
            style={{
              width: '100%',
              color: selectedKeys.length ? '#ff0066' : undefined,
              borderColor: selectedKeys.length ? '#ff0066' : undefined
            }}
          >
            {t('Reset')}
          </Button>
        </div>
      ),
      render: (type) => (
        <span style={{ textTransform: 'capitalize' }}>{type}</span>
      )
    },
    {
      title: t('Tokens'),
      dataIndex: 'token',
      key: 'token',
      sorter: true
    },
    {
      title: t('In Stock'),
      dataIndex: 'stock',
      key: 'stock',
      render: (stock, item) => (item.type === 'physical' ? stock : 'N/A'),
      sorter: true
    },
    {
      title: `${t('Is Publish')}?`,
      key: 'publish',
      dataIndex: 'publish',
      render: (text: boolean) => <span>{text ? t('YES') : t('NO')}</span>,
      filterIcon: (filtered) => (
        <SearchOutlined style={{ color: filtered ? '#ff0066' : undefined }} />
      ),
      filterDropdown: ({
        setSelectedKeys,
        selectedKeys,
        confirm,
        clearFilters
      }: any) => (
        <div style={{ padding: 5 }}>
          <Menu
            style={{ borderRight: 'none' }}
            onSelect={(param) => {
              setSelectedKeys([param.key]);
              confirm();
            }}
            selectedKeys={selectedKeys}
          >
            <Menu.Item key="true">{t('YES')} </Menu.Item>
            <Menu.Item key="false">{t('NO')}</Menu.Item>
          </Menu>
          <Button
            onClick={() => {
              setSelectedKeys([]);
              clearFilters();
            }}
            style={{
              width: '100%',
              color: selectedKeys.length ? '#ff0066' : undefined,
              borderColor: selectedKeys.length ? '#ff0066' : undefined
            }}
          >
            {t('Reset')}
          </Button>
        </div>
      )
    },
    {
      title: t('Action'),
      key: 'action',
      render: (product: IProduct) => {
        const { _id } = product;
        return (
          <Space size="middle">
            <Link
              as={`/account/performer/products/${_id}/update`}
              href={{
                pathname: '/account/performer/products/update',
                query: { id: _id, product: JSON.stringify(product) }
              }}
            >
              <a style={{ color: 'gray' }}>
                <EditOutlined />
              </a>
            </Link>
            <DeleteOutlined onClick={() => remove(_id)} />
          </Space>
        );
      }
    }
  ];
  const dataSource = products.map((p) => ({ ...p, key: p._id }));

  return (
    <Table
      dataSource={dataSource}
      columns={columns}
      className="table"
      pagination={{
        total,
        pageSize
      }}
      showSorterTooltip={false}
      loading={searching}
      onChange={onChange}
    />
  );
};
ProductsTable.defaultProps = {
  remove: null
};
export default ProductsTable;
