...
class SiderMenu extends PureComponent<IProps> {
  ...
  generateMenus = (data) => {
    const { totalNotReadMessage, t } = this.props;
    return data.map((item, index) => {
      if (item.children) {
        return (
          ...
          >
            {this.generateMenus(t(item.children))}
          </Menu.SubMenu>
        );
      }
      return (
  ...
  render() {
    const { theme, menus, collapsed, onClick, t } = this.props;
...
export default withRouter(withTranslation('components')(SiderMenu));
