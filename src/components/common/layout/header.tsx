...
class Header extends PureComponent<P, S> {
  ...
  handlePrivateChat = (data: { conversationId: string; user: IUser }) => {
    ...
    message.success({
      content: (
        <span>
          {data.user.username} {this.props.t('sent you a private chat request')}
          .
        </span>
      ),
      duration: 10,
  ...
  sendTipHandler = ({ token, senderInfo }) => {
    message.success(
      `${senderInfo.username} ${this.props.t('tip')} ${token?.toFixed(
        2
      )} ${this.props.t('tokens')}`,
      10
    );
      ...
      <>
        ...
        >
          ...
          >
            ...
            >
              {`${roundBalance(currentUser?.balance || 0)} ${this.props.t(
                'Tokens'
              )}`}
            </Button>
          </Tooltip>
            ...
            overlay={
              <Menu>
                {privateRequests.length > 0 ? (
                  ...
                  <Menu.Item>{t('There are no private request')}.</Menu.Item>
                )}
              </Menu>
            }
          >
          ...
          overlay={
            <Menu key="menu-right-content">
              {currentUser.role === 'user' && [
                ...
                >
                  <span>
                    <SettingOutlined className="primary-icon" /> {t('Profile')}
                  </span>
                </Menu.Item>,
                <Menu.Item
                ...
                >
                  <span>
                    <MessageOutlined className="primary-icon" /> {t('Messages')}{' '}
              ...
              {currentUser.role === 'performer' && [
                ...
                >
                  <span>
                    <UserOutlined className="primary-icon" /> {t('Profile')}
                  </span>
                </Menu.Item>,
                <Menu.Item
              ...
              <Menu.Item key="log-out" onClick={this.beforeLogout.bind(this)}>
                <LogoutOutlined className="primary-icon" /> {t('Log out')}
              </Menu.Item>
            </Menu>
          }
        >
      ...
      <Layout.Header className="header" id="layoutHeader">
        ...
        <div className="right-container">
          ...
          <div className={!showSearch ? 'hide search-bar' : 'search-bar'}>
            <Input.Search
              placeholder={this.props.t('Enter keyword')}
              loading={false}
              allowClear
              enterButton
...
export default connect(
  mapStateToProps,
  mapDispatch
)(withTranslation('components')(withRouter(Header)));
