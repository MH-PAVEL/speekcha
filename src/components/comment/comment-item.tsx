...
class CommentItem extends PureComponent<IProps> {
  ...
  render() {
    const { item, user, onDelete, commentMapping, t } = this.props;
    const fetchingComment = commentMapping.hasOwnProperty(item._id)
      ? commentMapping[item._id].requesting
      : false;
    const comments = commentMapping.hasOwnProperty(item._id)
      ? commentMapping[item._id].items
      : [];
    const totalComments = commentMapping.hasOwnProperty(item._id)
    ...
    return (
      <div>
        <div className="cmt-item" key={item._id}>
          <img
            alt="creator-avt"
            src={item?.creator?.avatar || '/no-avatar.png'}
          />
          <div className="cmt-content">
            <div className="cmt-user">
              <span>
                <span>
                  {item?.creator?.username || item?.creator?.name || 'N/A'}
                </span>
...
export default connect(
  mapStates,
  mapDispatch
)(withTranslation('components')(CommentItem));
