...
class MessageList extends PureComponent<IProps> {
  ...
  renderMessages = () => {
    ...
    if (!messages.length && !fetching) {
      const text = loggedIn ? (
        this.props.t('There are no chat')
      ) : (
        <>
          {this.props.t('There are no chat, please')}
          <Link href="/auth/register">
            <a> {this.props.t('register')} </a>
          </Link>
          {this.props.t('or')}
          <Link href="/auth/login">
            <a> {this.props.t('login')} </a>
          </Link>
          {this.props.t('to send message')}
        </>
      );
  ...
  render() {
    const { conversation, isPublic, message, t } = this.props;
    const { fetching } = message;
    if (!this.messagesRef) this.messagesRef = createRef();
    return (
      <div
        ...
        {conversation && conversation._id && (
          <>
            <div className="message-list-container" ref={this.messagesRef}>
              {fetching && <p className="text-center">{t('fetching')}...</p>}
              {this.renderMessages()}
            </div>
...
export default connect(
  mapStates,
  mapDispatch
)(withTranslation('components')(MessageList));
