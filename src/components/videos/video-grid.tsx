...
const VideoGrid = ({
  ...
  return (
    <InfiniteScroll pageStart={0} loadMore={addVideos} hasMore={hasMore}>
      <Card className="video-grid" title={t(title)} bordered={false}>
        {success &&
          (data.length > 0 ? (
            data.map((video: IVideo) => (
              <Card.Grid className="video-box" key={video._id}>
                ...
                <div className="video-info">
                  <div>
                    {t('Status')}:{' '}
                    <strong>{capitalizeFirstLetter(t(video.status))}</strong>
                  </div>
                  <Space>
                    <Link
                      href={{
