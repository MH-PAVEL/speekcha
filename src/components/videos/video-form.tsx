...
const VideoForm = ({ onFinish, loading, video }: IProps) => {
  ...
  if (video?.token && !unitPrices.find((p) => p.value === video.token)) {
    unitPrices.push({
      value: video.token,
      text: <NumberFormat value={video.token} suffix={t('tokens')} />
    });
  ...
  const rightInputFrom: FormItemProps[] = [
    {
      ...
      rules: [
        {
          required: true,
          message: t('Please input video token')
        },
        {
          validator: (_, value) =>
    ...
    },
    {
      ...
      rules: [
        {
          required: true,
          message: t('Please input video status')
        }
      ],
      children: (
        <Select placeholder={t('Select Video Status')}>
      ...
      <Row gutter={25}>
        <Col sm={12} xs={24}>
          ...
          <Form.Item
            name="video"
            label={t('Video')}
            rules={[{ required: true, message: t('File is required') }]}
          >
            <Upload
              showUploadList={{
          ...
          <Form.Item name="trailer" label={t('Trailer')}>
            <Upload
              showUploadList={{
                showRemoveIcon: false,
                showPreviewIcon: true,
